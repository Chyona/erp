...
export default function MonthEndClosingPanel({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const handleMarkDeclared = async () => {
      ...
      content: (
        <div>
          <Alert
            type="warning"
            showIcon
            message="标记后该季度为已申报状态，凭证不可增删改，且不可反结转。"
            style={{ marginBottom: 12 }}
          />
          <p style={{ marginBottom: 0 }}>
            期间：{summary.periodLabel}。请确认已向税务机关完成申报后再标记。
          </p>
        </div>
      ),
      okText: '确认已申报'
    ...
    try {
      ...
      message.success(`${summary.periodLabel} 已申报`);
      refresh();
      loadSummary();
    } catch (err) {
    ...
    <div className="tax-exemption-panel">
      ...
      {declared ? (
        <Alert
          ...
          showIcon
          style={{ margin: '12px 0' }}
          message={`${summary?.periodLabel || ''} 已申报（凭证不可增删改，不可反结转）。`}
        />
      ) : null}
