...
const QUICK_GUIDE = [
  ...
  },
  {
    ...
    tip: '2221 分录摘要写「销项税额（普票）」，月底可减免结转'
  },
  {
    scene: '收到开发款（开具专票）',
    type: '销售收入',
    account: '借 1002 / 贷 5001 + 2221',
    tip: '2221 分录摘要写「销项税额（专票）」，不参与减免结转'
  },
  {
    scene: '个人垫付',
    type: '日常费用',
    account: '借 5602 / 贷 2241 其他应付款',
    tip: '摘要末尾写（xx垫付），月底报销按此人汇总'
  },
  {
    scene: '月底还垫付',
    type: '日常费用',
    account: '借 2241 / 贷 1002',
    tip: '工作台「月底报销」按人一键生成'
  },
  {
  ...
  },
  {
    ...
    type: '工资薪酬',
    account: '2211 应付职工薪酬',
    tip: '工资可月底计提；社保、公积金无需计提，当月20日系统自动从公账扣缴'
  },
  {
    scene: '计提附加税（季度）',
    type: '税费缴纳',
    account: '借 5403 / 贷 2221',
    tip: '季末先计提附加税，再计提企业所得税，使季度利润更准确'
  },
  {
    scene: '缴纳增值税及附加税（季度）',
    type: '税费缴纳',
    account: '借 2221 / 贷 1002',
    tip: '附加税需先季末计提；与增值税一般一并申报打款'
  },
  {
    scene: '计提 / 缴纳企业所得税',
    type: '税费缴纳',
...
const REIMBURSE_FLOW = {
  title: '垫付怎么记、怎么还',
  steps: [
    '发生时：借 5602、贷 2241，摘要末尾写（xx垫付），如（thm垫付）',
    '系统从摘要识别垫付人，月底报销按 thm、zqn 等人分别汇总',
    '月底：工作台 → 月底报销 → 选月份 → 按人「生成还垫付凭证」',
    '已申报季度不再在此汇总，还垫付请凭证录入或导入'
  ],
  advanced: '格式示例：【办公费】腾讯云代码助手（thm垫付）'
};
...
const TAX_EXEMPTION_FLOW = {
  ...
  steps: [
    '销售凭证选「销售收入」，价税分离记账',
    '借 1002 银行存款，贷 5001 主营业务收入 + 2221 应交税费',
    '2221 分录摘要：普票写「销项税额（普票）」，专票写「销项税额（专票）」',
...
function FlowPanel({ flow, variant }: { flow: { title: string; steps: string[]; advanced: string }; variant?: string }) {
  return (
