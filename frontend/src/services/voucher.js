...
const STATUS = { DRAFT: 'draft', APPROVED: 'approved', LOCKED: 'locked' };
const STATUS_LABEL = { draft: '草稿', approved: '已审核', locked: '已锁定' };
const ATTACHMENT_READONLY_TIP = '已结账或已审核的凭证不支持上传、删除、修改附件';

function canModifyAttachments(status) {
  return status === STATUS.DRAFT;
}

function canEditVoucher(status) {
  return status === STATUS.DRAFT;
}

function isRedLetterVoucher(voucher) {
...
function generateChecksum(voucher) {
...
async function save(voucherData, approve = false) {
  ...
  voucherData.status = approve ? STATUS.APPROVED : STATUS.DRAFT;
  ...
  await DB.put('vouchers', voucherData);
...
async function unapproveMany(ids) {
  const uniqueIds = [...new Set(ids)];
...
async function findByVoucherNo(raw, { voucherType = '记', yearMonth } = {}) {
...
async function saveAttachment(file, customName) {
